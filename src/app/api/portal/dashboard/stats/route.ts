...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!token) {
      logger.warn('Dashboard stats unauthorized: no token', { endpoint: '/api/portal/dashboard/stats', method: 'GET' });
      return jsonResponse({ success: false, error: 'Unauthorized' }, 401);
    }

    const payload = verifyToken(token);
    if (!payload) {
      logger.warn('Dashboard stats unauthorized: invalid token', { endpoint: '/api/portal/dashboard/stats' });
